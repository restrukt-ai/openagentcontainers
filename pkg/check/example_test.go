...
// ExampleCheck_multipleIssues shows a manifest that triggers both a SeverityError and a
// SeverityWarning in a single Check call.
func ExampleCheck_multipleIssues() {
	labels := map[string]string{
		"org.openagentcontainers.version": "v1alpha1",
		"org.openagentcontainers.name":    "my-agent",
		// no description — SeverityWarning
		"org.openagentcontainers.orchestrator.env":              "ORCHESTRATOR_URL",
		"org.openagentcontainers.orchestrator.bearer.token.env": "ORCHESTRATOR_TOKEN",
		"org.openagentcontainers.mcp.calendar.bearer.token.env": "", // empty — SeverityError
	}
