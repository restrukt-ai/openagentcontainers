...
package openagentcontainers.v1alpha3;
...
- For each declared inference type, validate that at least one model available on the configured gateway satisfies all declared capability requirements (`context`, `reasoning`, `tools`, `input.*`, `output.*`); MUST fail deployment if no qualifying model exists (§7.2).
...
If no model available on the configured gateway satisfies all declared capability requirements for a declared inference type, the orchestrator MUST fail deployment before the container starts. The failure diagnostic MUST identify the inference type and the unsatisfied requirements. The orchestrator MUST NOT start the container and allow the harness to discover the unsatisfied requirements at runtime.
...
required structure — requires a major version increment. The current spec version is `v1alpha3`.
